document.addEventListener("DOMContentLoaded", function () {
    const form = document.getElementById("testo-form");
    if (!form) return;
    ...
    const previewWarning = document.getElementById("label-preview-warning");

    function updatePreview() {
        previewText.textContent = testoInput.value || "Testo dell'etichetta";
        previewText.style.fontSize = (fontSizeInput.value || 14) + "pt";
        const verticale = orientamentoSelect.value === "verticale";
        previewText.style.transform = verticale ? "rotate(-90deg)" : "none";

        const textSpan = previewText.scrollWidth;
        const limit = verticale ? previewBox.clientHeight : previewBox.clientWidth;
        const overflow = textSpan > limit;
        previewBox.classList.toggle("overflow", overflow);
        previewWarning.hidden = !overflow;
    }

    testoInput.addEventListener("input", updatePreview);
    fontSizeInput.addEventListener("input", updatePreview);
    orientamentoSelect.addEventListener("change", updatePreview);
    updatePreview();
